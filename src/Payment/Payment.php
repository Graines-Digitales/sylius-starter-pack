...
class Payment
{
    ...
    public function __construct(
        ...
        $shopID = $this->container->getParameter('shop_id');
        $certTest = $this->container->getParameter('shop_testing_key');
        $certProd = $this->container->getParameter('shop_production_key');
        $ctxMode = $this->container->getParameter('shop_context_mode');
        $platform = $this->container->getParameter('shop_platform_url');
        $debug = (isset($args['debug']))? $args['debug'] : false;

	    $this->algorithm = (isset($args['algorithm']))? $args['algorithm'] : 'sha1'; // sha1 | sha256
